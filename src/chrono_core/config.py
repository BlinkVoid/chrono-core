...
def default_workspace_root() -> str:
    """Workspace root, overridable via CHRONO_WORKSPACE_ROOT.

    Read per call (not at import) so the CLI and MCP server pick up the
    environment they were launched with. The former environment variable is
    accepted as a migration fallback.
    """
    return (
